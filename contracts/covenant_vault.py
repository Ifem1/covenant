...
class CovenantVault(gl.Contract):
    ...
    def deposit_bond(self,covenant_id):
        c=RegistryInterface(self.registry).view().get_covenant(covenant_id); assert RegistryInterface(self.registry).view().get_canonical_vault()==gl.message.contract_address and _record_field(c,'operator')==gl.message.sender_address and _record_field(c,'status')=='DRAFT' and gl.message.value>=_record_field(c,'minimum_bond'); assert self.bonds.get(covenant_id,u256(0))==0; self.principal[covenant_id]=gl.message.value; self.bonds[covenant_id]=gl.message.value; RegistryInterface(self.registry).emit(on='finalized').mark_funded(covenant_id)
    @gl.public.write
    def reconcile_funding(self,covenant_id: u256):
        c=RegistryInterface(self.registry).view().get_covenant(covenant_id); assert self.bonds.get(covenant_id,u256(0))>0; RegistryInterface(self.registry).emit(on='finalized').mark_funded(covenant_id) if _record_field(c,'status')=='DRAFT' else None
    @gl.public.write
    def apply_slash(self,covenant_id,audit_id):
        c=RegistryInterface(self.registry).view().get_covenant(covenant_id); a=RegistryInterface(self.registry).view().get_audit(audit_id); state=self.slash_state.get(audit_id,u32(0)); assert _record_field(a,'covenant_id')==covenant_id and _record_field(a,'outcome')=='BREACH' and state<3
        if state==0:
            used=self.consumed_slash_bps.get(covenant_id,u32(0))+_record_field(a,'slash_bps'); assert used<=10000; p=self.principal.get(covenant_id,u256(0)); target=p*used//10000; old=self.slashed_amount.get(covenant_id,u256(0)); amount=target-old; self.consumed_slash_bps[covenant_id]=used; self.slashed_amount[covenant_id]=target; self.bonds[covenant_id]-=amount; self.reserved_slash_amount[audit_id]=amount; self.pending_slash_count[covenant_id]=self.pending_slash_count.get(covenant_id,u32(0))+1; self.slash_state[audit_id]=u32(1); RegistryInterface(self.registry).emit(on='finalized').mark_audit_settled(audit_id); return
        if not _record_field(a,'settled'): RegistryInterface(self.registry).emit(on='finalized').mark_audit_settled(audit_id); return
        ...
        if amount>0: _Recipient(_record_field(c,'recovery')).emit_transfer(value=amount)
    @gl.public.write
    def close_and_refund(self,covenant_id):
        c=RegistryInterface(self.registry).view().get_covenant(covenant_id); assert _record_field(c,'operator')==gl.message.sender_address and _record_field(c,'status')=='EXPIRED' and self.pending_slash_count.get(covenant_id,u32(0))==0; state=self.close_state.get(covenant_id,u32(0));
        ...
        assert _record_field(RegistryInterface(self.registry).view().get_covenant(covenant_id),'status')=='CLOSED'; amount=self.bonds.get(covenant_id,u256(0)); self.bonds[covenant_id]=u256(0); self.close_state[covenant_id]=u32(2)
        if amount>0: _Recipient(_record_field(c,'operator')).emit_transfer(value=amount)
