...
class CovenantVault(gl.Contract):
    ...
    def deposit_bond(self,covenant_id):
        c=RegistryInterface(self.registry).view().get_covenant(covenant_id); assert RegistryInterface(self.registry).view().get_canonical_vault()==gl.message.contract_address and c.operator==gl.message.sender_address and c.status=='DRAFT' and gl.message.value>=c.minimum_bond; assert self.bonds.get(covenant_id,u256(0))==0; self.bonds[covenant_id]=gl.message.value; RegistryInterface(self.registry).emit(on='finalized').mark_funded(covenant_id)
    @gl.public.write
